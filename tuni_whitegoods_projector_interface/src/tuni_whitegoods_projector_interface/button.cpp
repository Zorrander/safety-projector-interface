...
cv::Mat Button::draw() {
  ...
  cv::Point text_origin(center_cam_point.x - textSize.width / 2,
                        center_cam_point.y + textSize.height / 2);

  cv::putText(btn_img, text, text_origin, fontFace, fontScale, txt_color,
              thickness);

  return btn_img;
}

bool Button::isAlreadyPressed() { return button_already_pressed; }
...
void Button::set_text_color(std_msgs::ColorRGBA text_color) {
  ...
  txt_color = cv::Scalar(text_color.b * 255, text_color.g * 255,
                         text_color.r * 255, text_color.a * 255);
}
