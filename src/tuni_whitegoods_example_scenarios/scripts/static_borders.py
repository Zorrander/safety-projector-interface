...
class Whitegoods_Static_Borders:
    def __init__(self):
        rospy.loginfo("Test HMI interface")
        self.go = False 
        self.stop = False 
        self.robot_border = 0

        rospy.loginfo("Loading preset ui server")
        preset_ui_server_name = "/execution/projector_interface/integration/actions/set_preset_ui_projection"
        self.preset_ui_client = actionlib.SimpleActionClient(preset_ui_server_name, SetPresetUIProjectionAction)
        ...
        self.layout_client = actionlib.SimpleActionClient(layout_server_name, SetLayoutStaticBordersAction)
        ...
        self.interaction_sub = rospy.Subscriber(interaction_topic, VirtualButtonEventArray, self.interaction_cb)

        border_violation_topic = "/execution/projector_interface/integration/topics/safety_border_violation"
        self.border_violation_sub = rospy.Subscriber(border_violation_topic, SafetyBorderViolation, self.border_violation_cb)

    def interaction_cb(self, msg):
        for btn_event in msg.virtual_button_events:
            rospy.loginfo(btn_event)
    ...
    def task_place_object(self):
        ...
        robot_booking_goal = BookRobotStaticBorderGoal()
    ...
    def project_buttons(self):
        preset_ui_goal = SetPresetUIProjectionGoal()
          
        button_1 = button_2 = VirtualButtonReference()
    ...
    def project_static_borders(self):
        ...
        self.border_client = actionlib.SimpleActionClient(border_server_name, SetSafetyBorderProjectionAction)
        ...
        rospy.loginfo("Loading border booking and releasing servers")
        book_robot_static_border_client = actionlib.SimpleActionClient(book_robot_static_border_server_name, BookRobotStaticBorderAction);
        book_robot_static_border_client.wait_for_server()

        release_robot_static_border_client = actionlib.SimpleActionClient(release_robot_static_border_server_name, ReleaseRobotStaticBorderAction);
        release_robot_static_border_client.wait_for_server()

        book_operator_static_border_client = actionlib.SimpleActionClient(book_operator_static_border_server_name, BookOperatorStaticBorderAction);
        book_operator_static_border_client.wait_for_server()

        release_operator_static_border_client = actionlib.SimpleActionClient(release_operator_static_border_server_name, ReleaseOperatorStaticBorderAction);
        release_operator_static_border_client.wait_for_server()
...
if __name__ == "__main__":
    rospy.init_node('whitegoods_static_borders')
