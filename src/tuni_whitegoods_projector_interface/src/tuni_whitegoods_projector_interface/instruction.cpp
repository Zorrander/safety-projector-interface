...
public:
    Instruction(int id_, int zone, const Point &target, const string &title, const Scalar &title_c,
                const string &description, const Scalar &desc_c, int lifetime)
        : _id(id_), _zone(zone), _target(target), _title(title),
          _title_c(title_c * 255), _desc(description), _desc_c(desc_c * 255), _lifetime(lifetime) {}
    ...
    Mat draw_instruction(Mat &img) const {
        ...
        int baseline = 0;
        Size text_size_title = getTextSize(_title, TEXT_FACE, TEXT_SCALE_TITLE, TEXT_THICKNESS, &baseline);
        Size text_size = getTextSize(_desc, TEXT_FACE, TEXT_SCALE, TEXT_THICKNESS, &baseline);
        int tot_height = text_size_title.height + text_size.height;

        int start_title_x = _target.x + (text_size.width - text_size_title.width) / 2;
        int start_title_y = _target.y - (text_size.height - 10);
        Point pos_desc(_target.x, _target.y + text_size_title.height);

        putText(img, _title, Point(start_title_x, start_title_y), TEXT_FACE, TEXT_SCALE_TITLE, _title_c, TEXT_THICKNESS, LINE_AA);
        putText(img, _desc, pos_desc, TEXT_FACE, TEXT_SCALE, _desc_c, TEXT_THICKNESS, LINE_AA);
        ...
        img = draw_border(img, tl_corner, br_corner, _title_c, 2, 10, 20); // 80 bracket style

        return img;
    }

private:
    Mat draw_border(Mat &img, const Point &pt1, const Point &pt2, const Scalar &color, int thickness, int r, int d) const {
