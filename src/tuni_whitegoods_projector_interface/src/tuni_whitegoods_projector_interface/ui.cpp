...
class InterfaceUI {
public:
    InterfaceUI(const std::string& resource_id, const std::string& zone, const std::string& name, const std::string& description, const std::vector<Button>& buttons)
        : resource_id(resource_id), zone(zone), name(name), description(description), hidden(false), modified_interface(true) {
        if (!buttons.empty()) {
    ...
    const std::vector<Button>& get_list_buttons() const {
        return list_buttons;
    }
    ...
    cv::Mat get_image_interface() const {
        return interface_img;
    }
