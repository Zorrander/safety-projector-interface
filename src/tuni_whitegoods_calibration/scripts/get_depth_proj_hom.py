...
class DepthHom():
	def __init__(self):
		...
		self.folder = self.name_f.parent / "homography_test" 
		self.wait = True
		self.tags = []
		self.size = 50
		self.counter = 0
	...
	def display_grid(self):
		...
		for y in range(580,1080,100):
			for x in range(620,1620,100):
...
#		val = input("Excluded values: ")
	...
	for proj in projectors:
		name_cp = str(dh.folder) + "/proj_{}_points"
		name_cd = str(dh.folder) + "/dm_{}_points"
		...
		M = cv2.findHomography(coords_depth, coords_proj, cv2.RANSAC, 5.0)[0]
		name_save = str(dh.folder) + "/depth_proj_{}"
