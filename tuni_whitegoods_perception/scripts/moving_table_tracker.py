   ...
   # subscriber that get the RGB image
    ...
    def find_dynamic_ui_transform(self, rgb_img, depth_image):
        ...
        if len(corners) == 0:
            ...
            tl_3D_coordinates = self.project_pixel_to_3D(
                topLeft[0], topLeft[1], table_z)
            tr_3D_coordinates = self.project_pixel_to_3D(
                tr_point_t[0], tr_point_t[1], table_z)
            br_3D_coordinates = self.project_pixel_to_3D(
                br_point_t[0], br_point_t[1], table_z)
            bl_3D_coordinates = self.project_pixel_to_3D(
                bl_point_t[0], bl_point_t[1], table_z)

            # Create the input pose stamped message
            self.create_input_message(tl_3D_coordinates)
            # Perform the transformation
            top_left_robot_coordinates = self.transform_world_coordinates(
                self.in_point_stamped, "base")

            self.create_input_message(tr_3D_coordinates)
            top_right_robot_coordinates = self.transform_world_coordinates(
                self.in_point_stamped, "base")

            self.create_input_message(br_3D_coordinates)
            bottom_right_robot_coordinates = self.transform_world_coordinates(
                self.in_point_stamped, "base")

            self.create_input_message(bl_3D_coordinates)
            bottom_left_robot_coordinates = self.transform_world_coordinates(
                self.in_point_stamped, "base")
            ...
            marker.points.append(
                top_right_robot_coordinates.out_point_stamped.pose.position)
            marker.points.append(
                bottom_right_robot_coordinates.out_point_stamped.pose.position)
            marker.points.append(
                bottom_left_robot_coordinates.out_point_stamped.pose.position)
            marker.points.append(
                top_left_robot_coordinates.out_point_stamped.pose.position)
