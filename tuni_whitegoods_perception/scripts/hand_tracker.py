...
class HandTracker(object):
   def __init__(self, mode=False, maxHands=2, detectionCon=0.5, modelComplexity=0, trackCon=0.5):
      ...
      self.pub_hands_poi = rospy.Publisher("/odin/internal/hand_detection", HandsState, queue_size=1)
      ...
      self.sync = message_filters.ApproximateTimeSynchronizer([self.rgb_sub, self.depth_sub], queue_size=1, slop=0.1)
   ...
   def callback_image(self, msg, depth_msg, draw=False):
      ...
      if self.results.multi_handedness:
          ...
          if draw:
            cv2.imshow("Hand Tracker", rgb_img)  # Display the image with hand position
            cv2.waitKey(1)
