...
CameraView::CameraView(ros::NodeHandle* nh) : it_(*nh), nh_(nh) {
  ...
  img_callback = it_.subscribe("/camera1/depth_to_rgb/image", 10,
                               &CameraView::depthSceneCallback, this);
...
void CameraView::updateBorders(
  ...
  for (auto& border : borders) {
    int grayscale_intensity =
        static_cast<int>((border->border_color.r + border->border_color.g +
                          border->border_color.b) /
                         3 * 255);

    // Draw the rectangle with a single intensity value (grayscale)
    cv::rectangle(
        *layer, border->top_left_cam_point, border->bottom_right_cam_point,
        cv::Scalar(grayscale_intensity), border->thickness * 2, cv::LINE_8);
  }
}
...
void CameraView::updateDisplayAreas(
  ...
  for (auto& zone : zones) {
    layers[zone->name] = std::make_shared<cv::Mat>(
        cv::Mat::zeros(camera_resolution[1], camera_resolution[0], CV_8UC1));
...
void CameraView::publish_image() {
  ...
  camera_viz_msg =
      cv_bridge::CvImage(std_msgs::Header(), "mono8", combined).toImageMsg();
...
void CameraView::depthSceneCallback(
  ...
  cv::Mat depth_image = bridge_cv_depth->image;

  cv::Mat depth_normalized;
  cv::normalize(depth_image, depth_normalized, 0, 255, cv::NORM_MINMAX, CV_8U);

  *layers["background"] = depth_normalized;
}
