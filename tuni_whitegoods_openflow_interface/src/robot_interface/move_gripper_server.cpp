...
MoveGripperServer::MoveGripperServer(ros::NodeHandle *nh_, std::string name_mg)
    :  // Bind the callback to the action server. False is for thread spinning
      as_move_gripper(
          *nh_, name_mg,
          boost::bind(&MoveGripperServer::executeMoveGripper, this, _1), false),
  ...
  client_load = nh_->serviceClient<ur_dashboard_msgs::Load>(
      "/ur_hardware_interface/dashboard/load_program");
  client_play = nh_->serviceClient<std_srvs::Trigger>(
      "/ur_hardware_interface/dashboard/play");
}

// move the joints of the robot to some positions
void MoveGripperServer::executeMoveGripper(
    const ControlOpenCloseToolGoalConstPtr &goal) {
  bool success = true;
  /*
