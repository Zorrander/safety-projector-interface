...
MoveRobotServer::MoveRobotServer(ros::NodeHandle *nh_, std::string name_mj)
    :  // Bind the callback to the action server. False is for thread spinning
      as_move_joints(*nh_, name_mj,
                     boost::bind(&MoveRobotServer::executeMoveJoints, this, _1),
                     false),
      action_name_move_joints(name_mj) {
  pub_cmd_robot = nh_->advertise<std_msgs::String>(
...
void MoveRobotServer::executeMoveJoints(const MoveJointsGoalConstPtr &goal) {
  ...
  std::string cmd =
      "movej([" + std::to_string(goal->trajectory.points[0].positions[0]) +
      "," + std::to_string(goal->trajectory.points[0].positions[1]) + "," +
      std::to_string(goal->trajectory.points[0].positions[2]) + "," +
      std::to_string(goal->trajectory.points[0].positions[3]) + "," +
      std::to_string(goal->trajectory.points[0].positions[4]) + "," +
      std::to_string(goal->trajectory.points[0].positions[5]) +
      "], a=0.3, v=0.13)";
