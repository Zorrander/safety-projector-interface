...
BookOperatorStaticBorderServer::BookOperatorStaticBorderServer(
    ros::NodeHandle* nh_, std::string name_book_operator,
    ...
    :  // Bind the callback to the action server. False is for thread spinning
       // nh_border_(*n),
      as_book_operator(
          *nh_, name_book_operator,
          boost::bind(&BookOperatorStaticBorderServer::executeBookOperator,
                      this, _1),
          false),
...
// send feedback
void BookOperatorStaticBorderServer::sendFeedbackBookOperator(
    std::string req_id) {
...
// send result
void BookOperatorStaticBorderServer::sendResultBookOperator(
    std::string req_id) {
