...
UnsetProjectionServer::UnsetProjectionServer(
    ros::NodeHandle *nh_, std::string name_un,
    std::shared_ptr<ProjectorInterfaceController>
        projector_interface_controller)
    : as_unset(*nh_, name_un,
               boost::bind(&UnsetProjectionServer::executeUnsetUI, this, _1),
               false),
...
void UnsetProjectionServer::executeUnsetUI(
    const UnsetProjectionGoalConstPtr &goal) {
  bool success = true;
  sendFeedBackUnset();
