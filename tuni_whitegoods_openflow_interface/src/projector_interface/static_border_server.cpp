...
StaticBorderServer::StaticBorderServer(
    ros::NodeHandle *nh, std::string name_bd_manager,
    ...
    :  // Bind the callback to the action server. False is for thread spinning
      as_border_manager(
          *nh, name_bd_manager,
          boost::bind(&StaticBorderServer::executeSetLayout, this, _1), false),
...
void StaticBorderServer::executeSetLayout(
    const SetLayoutStaticBordersGoalConstPtr &goal) {
  bool success = true;

  controller->createBorderLayout(goal->size_rows, goal->size_cols,
                                 goal->safety_factor, goal->book_adjacent,
                                 goal->status_booked, goal->status_free,
                                 goal->status_operator);
