...
ReleaseOperatorStaticBorderServer::ReleaseOperatorStaticBorderServer(
    ...
    :  // Bind the callback to the action server. False is for thread spinning
      as_release_operator(
          *nh_, name_release_operator,
          boost::bind(
              &ReleaseOperatorStaticBorderServer::executeReleaseOperator, this,
              _1),
          false),
...
void ReleaseOperatorStaticBorderServer::executeReleaseOperator(
...
// send feedback
void ReleaseOperatorStaticBorderServer::sendFeedbackReleaseOperator(
    std::string req_id) {
...
// send result
void ReleaseOperatorStaticBorderServer::sendResultReleaseOperator(
    std::string req_id) {
