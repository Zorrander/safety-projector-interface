...
UserInterfaceServer::UserInterfaceServer(
    ros::NodeHandle *nh_, std::string name_pre,
    std::shared_ptr<ProjectorInterfaceController>
        projector_interface_controller)
    : as_preset_ui(*nh_, name_pre,
                   boost::bind(&UserInterfaceServer::executePresetUI, this, _1),
                   false),
...
void UserInterfaceServer::executePresetUI(
    const SetPresetUIProjectionGoalConstPtr &goal) {
  int counter = 0;
