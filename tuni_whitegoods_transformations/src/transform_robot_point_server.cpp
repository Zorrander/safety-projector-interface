...
/**
 * @brief      This class describes a transform robot point server.
 */
 ...
 public:
  ...
  TransformRobotPointServer(ros::NodeHandle* nh)
      : nh_(nh),
        ...
        tfListener(new tf2_ros::TransformListener(
            tfBuffer))  // Initialize tfListener with tfBuffer
  {
  ...
  bool transformWorldPointCallback(
    ...
    if (tfBuffer.canTransform(req.target_frame,
      ...
      try {
        ...
        /*
        ROS_INFO("After transformation: translation (%.2f, %.2f, %.2f), rotation
        (%.2f, %.2f, %.2f, %.2f)", res.out_point_stamped.pose.position.x,
